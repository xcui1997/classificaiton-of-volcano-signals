...
# -*- encoding: utf-8 -*-
'''
@文件        :hawaii_volcano.py
@说明        :
@时间        :2021/01/20 13:48:23
@作者        :xcui
@版本        :1.0
'''
...
def read_name(STR):
    ...
    name_event = STR.split('/')[-1]
    name_event = name_event.split('.')[0]
    name_event = '-'.join(name_event.split('-')[0:3])
    name_event = name_event[0:4] + '-' + name_event[4:6] + '-' + name_event[6:8] + '-'  \
        + name_event[8:10] + ':' + name_event[10:12] + ':' + name_event[12:14] \
        + '.' + name_event[14:17]
    return name_event


def time_sort(name_all, amp_all, fi_all):
    Time = []
    for i in range(len(name_all)):
        Time.append(UTCDateTime(name_all[i]))
    idx = np.argsort(np.asarray(Time))
    return np.asarray(name_all)[idx], np.asarray(amp_all)[idx], np.asarray(fi_all)[idx]
...
# Re-find the first arrival of p-wave.
...
# calculate the Euclidean distance between spectra. 
...
# Show the histogram of classification results. 
...
# Plot 100 spectra of each clusters.
...
if __name__ == '__main__':
    ...
    parser = ap.ArgumentParser(
        prog='volcanic_signals_classification.py',
    ...
    for sacname in glob.glob(params["data_dir"]+"/*.SAC"):
        tr = read(sacname)[0]
        ...
        if p_arr > win_snr and p_arr < t[-1]-win_snr and quality > snr:
            ...
            if snr_value > snr:
                ...
                name_event = read_name(sacname)
                amp_qc.append(Amp_nmlz)
                fi_qc.append(fi)
                name_qc.append(name_event)


###########################################################################
    #time in order
    name_qc, amp_qc, fi_qc = time_sort(name_qc, amp_qc, fi_qc)
    #calculate
    median_distance = calculate_EM(amp_qc)
    ...
    # plot mean spectra
    mean_amp_container, sort_idx = freq_sort(clust.labels_, amp_qc, params["n_cls"])
    len_idx = plot_mean_spectra(clust.labels_, amp_qc, median_distance, mean_amp_container, sort_idx)
    #plot 100 spectra
    plot_rep(clust, amp_qc, sort_idx)
    # plot freq-evengy space
    freq_energy_distribution(mean_amp_container, freq, sort_idx, len_idx)
    #save files
    new_catalog(clust.labels_, event_info, name_qc, fi_qc, sort_idx)
